...
public sealed class BlogFavoriteFolderTests
{
    ...
    {
        var id = BlogFavoriteFolderIdentity.New();
        var ownerId = UserIdentity.New();
        var name = new CollectionFolderName("Reading");

        var folder = BlogFavoriteFolder.Create(
            id,
            ownerId,
            name,
            sortOrder: 3,
    ...
    public void Rename_WithValidTime_ReplacesName()
    {
        ...
        var name = new CollectionFolderName("Favorites");
    ...
    {
        Assert.Throws<ArgumentOutOfRangeException>(
            () => BlogFavoriteFolder.Reconstitute(
    ...
    private static BlogFavoriteFolder CreateFolder(
