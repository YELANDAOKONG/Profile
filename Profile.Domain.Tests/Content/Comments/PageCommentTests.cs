...
public sealed class PageCommentTests
{
    ...
    {
        var id = PageCommentIdentity.New();
        ...
        var comment = PageComment.Create(
            id,
            authorId,
            pageId,
            null,
        ...
        Assert.Equal(pageId, comment.PageId);
    ...
    {
        var comment = CreateComment(
            body: CreateBody(new string('a', PageComment.MaximumBodyLength)));

        Assert.Equal(PageComment.MaximumBodyLength, comment.Body?.Source.Length);
    }
    ...
    public void Create_WithBodyOverMaximumLength_ThrowsArgumentOutOfRangeException()
    {
        Assert.Throws<ArgumentOutOfRangeException>(
            () => CreateComment(
                body: CreateBody(
                    new string('a', PageComment.MaximumBodyLength + 1))));
    }

    [Fact]
    public void Create_WithMaximumMediaCount_AllowsValue()
    {
        var media = Enumerable.Range(0, PageComment.MaximumMediaCount)
            .Select(static _ => CreateMedia())
            .ToArray();

        Assert.Equal(
            PageComment.MaximumMediaCount,
    ...
    {
        var media = Enumerable.Range(0, PageComment.MaximumMediaCount + 1)
            .Select(static _ => CreateMedia())
            .ToArray();
    ...
    {
        var pageId = PageIdentity.New();
        var parent = ReconstituteAtStatus(parentStatus, pageId: pageId);

        var reply = CreateComment(pageId: pageId, parentComment: parent);
        ...
        CommentStatus parentStatus)
    {
        ...
        Assert.Throws<InvalidOperationException>(
            () => CreateComment(pageId: pageId, parentComment: parent));
    }
    ...
    public void CreateReply_WithOwnIdentityAsParent_ThrowsArgumentException()
    {
        ...
        Assert.Throws<ArgumentException>(
            () => CreateComment(id, pageId, parent));
    }
    ...
    {
        Assert.Throws<ArgumentException>(
            () => PageComment.Reconstitute(
                PageCommentIdentity.New(),
                UserIdentity.New(),
                PageIdentity.New(),
                null,
                CreateBody(),
    ...
    private static PageComment CreateComment(
        PageCommentIdentity? id = null,
        PageIdentity? pageId = null,
        PageComment? parentComment = null,
        ...
        PageComment.Create(
            id ?? PageCommentIdentity.New(),
            UserIdentity.New(),
            pageId ?? PageIdentity.New(),
            parentComment,
            body ?? CreateBody(),
            media ?? [],
            policy,
    ...
    private static PageComment ReconstituteAtStatus(
        CommentStatus status,
        PageCommentIdentity? id = null,
        PageIdentity? pageId = null,
        IEnumerable<MediaReference>? media = null) =>
