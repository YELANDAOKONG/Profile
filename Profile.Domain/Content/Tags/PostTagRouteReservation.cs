...
public sealed record PostTagRouteReservation
{
    public const int DefaultReservationPeriodDays = 90;

    public PostTagRouteReservation(
        PostTagIdentity tagId,
        ...
        DateTimeOffset releasesAt)
    {
        ...
        if (releasesAt <= reservedAt)
        {
            throw new ArgumentOutOfRangeException(
                nameof(releasesAt),
                releasesAt,
                "Post tag route release time must be later than its reservation time.");
        }
    ...
    public PostTagIdentity TagId { get; }
    ...
    public static PostTagRouteReservation Create(
        PostTagIdentity tagId,
        UserIdentity ownerId,
        TaxonomyRouteIdentifier routeIdentifier,
        DateTimeOffset reservedAt,
        TimeSpan reservationPeriod)
    {
        if (reservationPeriod <= TimeSpan.Zero)
        {
            throw new ArgumentOutOfRangeException(
                ...
                reservationPeriod,
                "Post tag route reservation period must be greater than zero.");
        }

        return new PostTagRouteReservation(
            tagId,
            ownerId,
            routeIdentifier,
            reservedAt,
